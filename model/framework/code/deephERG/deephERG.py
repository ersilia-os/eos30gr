...
trainingset = dc.utils.data_utils.load_from_disk(trainingset_file) # save
...
testset = dc.utils.data_utils.load_from_disk(testset_file)
...
validationset = dc.utils.data_utils.load_from_disk(validationset_file) # save
...
def transformer(dataset_train,dataset_test,dataset_validation):
    transformers_train = [dc.trans.BalancingTransformer(dataset=dataset_train)]
    ...
    transformers_test = [dc.trans.BalancingTransformer(dataset=dataset_test)]
    print("About to transform test data")
    for transformer in transformers_test:
        dataset_test = transformer.transform(dataset_test)

    transformers_validation = [dc.trans.BalancingTransformer(dataset=dataset_validation)]
...
if method =="1" or method =="3":
    ...
    loader = dc.data.UserCSVLoader(tasks=tasks, feature_field='Smiles', id_field="No.", featurizer=featurizer)
    ...
    for trial in range(num_trials):
        ...
        def model_builder(): #model_params, model_dir
            model = dc.models.MultitaskClassifier( # TensorflowMultiTaskClassifier
                n_tasks=len(tasks),
